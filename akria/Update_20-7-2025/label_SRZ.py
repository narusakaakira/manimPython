...
class SZRExplanation(Scene):
    def construct(self):
        ...
        eq_Z=eq_Z.move_to(UP * 3)
        self.play(ReplacementTransform(eq_S, eq_Z))
        self.show_z_derivative()
        eq_R = eq_R.move_to(UP*3)
        self.play(ReplacementTransform(eq_Z, eq_R))
        self.show_r_derivative()
    ...
    def show_beta_sz_explanation(self):
        steps = [
            ...
            r"\text{Tốc độ lây nhiễm: } (\beta N Z) \cdot P(S) = \beta S Z\text{(giảm)}",
        ]
    ...
    def show_delta_s_explanation(self):
        steps = [
            ...
            r"\text{Tốc độ tử vong tự nhiên của dân số: } \delta S \text{(giảm)}"
        ]
        step_mobjects = [MathTex(s, tex_template=my_template, font_size=36) for s in steps]
    ...
    def show_z_derivative(self):
        steps = [
            r"\text{Bị lây nhiễm bởi người khỏe mạnh, ta sẽ có tốc độ lây nhiễm là $\beta S Z$ (tăng)}",
            r"\text{Bị lây nhiễm bởi người đã chết ta sẽ có tốc độ lây nhiễm là $\zeta R$ (tăng)}",
            r"\text{Bị tiêu diệt bởi người khỏe mạnh. Bằng cách định nghĩa tương tự với $\beta SZ$} ",
            r"\text{ta sẽ biết được tốc độ Zombie bị tiêu diệt trên đơn vị thời gian là $\alpha SZ$ (giảm)}"
        ]
        step_mobjects = [MathTex(s, tex_template=my_template, font_size=36) for s in steps]
        vgroup = VGroup(*step_mobjects).arrange(DOWN, buff=0.4).move_to(ORIGIN)
        ...
        self.wait(1)
        self.play(FadeOut(vgroup))
    def show_r_derivative(self):
        steps = [
            r"\text{Tốc độ tử vong của người khỏe mạnh trên đơn vị thời gian là $\delta S$ (tăng)}",
            r"\text{Tốc độ Zombie bị tiêu diệt trên đơn vị thời gian là $\alpha SZ$ (tăng)}",
            r"\text{Tốc độ lây nhiễm là $\zeta R$ (giảm)} ",
        ]
        step_mobjects = [MathTex(s, tex_template=my_template, font_size=36) for s in steps]
        vgroup = VGroup(*step_mobjects).arrange(DOWN, buff=0.4).move_to(ORIGIN)
